 ...
 **/
public class HadoopChmodStep extends BaseStep implements StepInterface {
    private static final Class<?> PKG = HadoopChmodStep.class; // for i18n purposes
    ...
    private static Integer privigeIndex=-1;
    public HadoopChmodStep(StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta, Trans trans) {
    ...
    public boolean init(StepMetaInterface smi, StepDataInterface sdi ) {
        ...
        HadoopChmodMeta meta = (HadoopChmodMeta) smi;
    ...
    public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws KettleException {
        ...
        if ( first ) {
            ...
            privigeIndex = data.outputRowMeta.indexOfValue( meta.getHadoop().getPrivige());
            if (userIndex < 0 ||pathIndex<0||uriIndex<0||outputIndex<0||privigeIndex<0) {
        ...
        try{
            ...
            String privige= (String)outputRow[privigeIndex];
//            String output= (String)outputRow[outputIndex];
            HadoopBean bean=new HadoopBean();
            bean.setUri(uri);
            bean.setPath(path);
            bean.setUser(user);
            bean.setPrivige(privige);
            String str=HdfsUtil.chmod(bean);
